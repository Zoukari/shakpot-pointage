...
async function initAdmin() {
  ...
  try {
    ...
    console.error("Erreur renderCurrentHourlyCode:", e);
  ...
  try {
    ...
    console.error("Erreur renderDevicesList:", e);
    const list = document.getElementById("devicesList");
    if (list) list.innerHTML = `<div class="empty-state" style="color:var(--red-text);">Erreur de chargement : ${e.message || e}</div>`;
  }
...
function computeWorkedHoursWithAdjustments(employeeId, start, end, logs, dayAdjustmentsMap) {
  ...
  if (!hasAnyAdjustment) {
    // Aucune correction manuelle : calcul simple sur toute la plage (gère bien les shifts minuit)
    return computeWorkedHours(logs, employeeId, start, end);
  }
  ...
  empLogs.forEach(log => {
    ...
    if (log.type === "in") {
      ...
      const duration = (t - openIn) / 3600000;
      // Si le jour de l'entrée a un ajustement manuel, on ignore cette paire (elle sera remplacée)
