    ...
    list.innerHTML = employees.map(emp => `
      <div class="emp-list-row">
        <div>
          ...
          ${emp.hourly_rate ? `<span class="emp-pin">${emp.hourly_rate}€/h</span>` : ''}
...
function getPeriodRangeFor(typeFieldId, dateFieldId) {
  ...
  if (type === "all") {
    ...
    // Pour les shifts nocturnes (ex: 17h→01h30), on étend jusqu.au lendemain 8h
    // pour capturer les sorties post-minuit sans mordre sur le soir suivant
    start = new Date(date);
    end = new Date(date);
  ...
  empLogs.forEach(log => {
    ...
    if (log.type === "in") {
      ...
      if (allShifts && allShifts.length > 0) {
        ...
        totalMs += (t - openIn); // fallback sans planning
      }
      openIn = null;
    }
  });
...
async function renderHoursTable() {
  ...
  // Pour les shifts nocturnes, on étend la fin de la requête au lendemain 8h
  // afin de capturer les sorties post-minuit (ex: entrée 01/07 17h → sortie 02/07 01h54)
  const endExtended = new Date(end);
  if (periodType === "day") {
    endExtended.setHours(8, 0, 0, 0); // déjà fait dans getPeriodRange, mais on s'assure
  } else if (periodType === "week" || periodType === "month") {
    // Pour semaine/mois, on ajoute 13h à la fin pour attraper la dernière nuit
    endExtended.setHours(endExtended.getHours() + 8);
  }
  ...
  for (const emp of employees) {
    ...
    const worked = computeWorkedHoursWithAdjustments(emp.id, start, end, logs, dayAdjustmentsMap, allShifts);
...
async function renderMyspaceHours() {
  ...
  const endExtended = new Date(end.getTime() + 8 * 3600000);
  ...
  const allShifts = await loadAllShifts();
  const worked = computeWorkedHoursWithAdjustments(myspaceEmployee.id, start, end, logs || [], dayAdjustmentsMap, allShifts);
...
function populateChargeEmployeeSelect() {
  ...
  if (!sel) return;
  sel.innerHTML = employees.map(e => `<option value="${e.id}" data-rate="${e.hourly_rate || ''}">${e.full_name}${e.hourly_rate ? ' (' + e.hourly_rate + '€/h)' : ''}</option>`).join("");
}
...
async function onChargeEmployeeChange() {
  ...
  info.textContent = `Calcul auto : ${fmtHours(workedH)} × ${rate}€ = ${autoSalary}€`;
...
async function saveCA() {
  ...
  status.textContent = `✓ CA du ${dateVal} enregistré : ${amount}€`;
  loadAccountingDay();
  renderAccountingSummary();
}
...
async function loadAccountingDay() {
  const dateVal = document.getElementById("accountingDate").value;
  if (!dateVal) return;
  const [purchases, charges, revenue] = await Promise.all([
  ...
  if (revenue.data) document.getElementById("caAmount").value = revenue.data.amount;
...
async function renderAccountingSummary() {
  ...
  const [purchasesRes, chargesRes, revenueRes] = await Promise.all([
    sb.from("accounting_purchases").select("*").gte("entry_date", startStr).lt("entry_date", endStr),
    sb.from("accounting_charges").select("*, employees(full_name)").gte("entry_date", startStr).lt("entry_date", endStr),
    sb.from("accounting_revenue").select("*").gte("entry_date", startStr).lt("entry_date", endStr)
  ]);
  ...
  let html = `
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;margin-bottom:20px;">
      <div style="background:var(--green-bg);border-radius:12px;padding:16px;text-align:center;">
        ...
        <div style="font-size:28px;font-weight:800;color:var(--green);">${totalCA.toFixed(2)}€</div>
      </div>
      <div style="background:var(--red-bg);border-radius:12px;padding:16px;text-align:center;">
        ...
        <div style="font-size:28px;font-weight:800;color:var(--red-text);">${totalPurchases.toFixed(2)}€</div>
      </div>
      <div style="background:var(--red-bg);border-radius:12px;padding:16px;text-align:center;">
        <div style="font-size:13px;color:var(--red-text);font-weight:700;">💳 Charges</div>
        <div style="font-size:28px;font-weight:800;color:var(--red-text);">${totalCharges.toFixed(2)}€</div>
      </div>
      <div style="background:${netProfit >= 0 ? 'var(--green-bg)' : 'var(--red-bg)'};border-radius:12px;padding:16px;text-align:center;grid-column:1/-1;">
        ...
        <div style="font-size:36px;font-weight:800;color:${netProfit >= 0 ? 'var(--green)' : 'var(--red-text)'};">${netProfit >= 0 ? '+' : ''}${netProfit.toFixed(2)}€</div>
  ...
  if (purchases.length > 0) {
    html += `<h4 style="color:var(--bordeaux-dark);margin-bottom:8px;">Détail des courses</h4>
    <table><tr><th>Date</th><th>Magasin</th><th>Montant</th><th>Facture</th></tr>`;
    purchases.forEach(p => {
      html += `<tr>
        <td>${p.entry_date}</td><td>${p.store_name}</td>
  ...
  if (charges.length > 0) {
    ...
    <table><tr><th>Date</th><th>Type</th><th>Détail</th><th>Montant</th></tr>`;
    charges.forEach(c => {
...
async function loadMyspaceAccountingDay() {
  ...
  if (revenue.data) document.getElementById("myspaceCAAmount").value = revenue.data.amount;
  else document.getElementById("myspaceCAAmount").value = "";

  const recap = document.getElementById("myspaceDayRecap");
  const ps = purchases.data || [];
  if (ps.length === 0) { recap.innerHTML = ""; return; }
  const total = ps.reduce((s, p) => s + parseFloat(p.amount), 0);
  recap.innerHTML = `<div style="font-weight:700;margin-bottom:6px;">Courses saisies aujourd'hui : <span style="color:var(--bordeaux)">${total.toFixed(2)}€</span></div>
    ${ps.map(p => `<div style="font-size:13px;color:var(--gray);">${p.store_name} — ${parseFloat(p.amount).toFixed(2)}€</div>`).join("")}`;
}

async function saveMyspaceCA() {
  const dateVal = document.getElementById("myspaceAccountingDate").value;
  const amount = parseFloat(document.getElementById("myspaceCAAmount").value);
  if (!dateVal || isNaN(amount)) { alert("Merci de saisir un montant."); return; }
  const { error } = await sb.from("accounting_revenue").upsert(
    { entry_date: dateVal, amount, entered_by: myspaceEmployee.id },
    { onConflict: "entry_date" }
  );
  if (error) { alert("Erreur: " + error.message); return; }
  document.getElementById("myspaceCAStatus").textContent = `✓ CA enregistré : ${amount}€`;
}
...
async function saveMyspacePurchase() {
  ...
  if (!store || isNaN(amount)) { alert("Merci de remplir le magasin et le montant."); return; }
